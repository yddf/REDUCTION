pro optordv,data,modl,munc,int,sint,sig = sig, sky = sky, gain=gain, ron=ron, changes = changes, stop=stop
...
;             will be set by the global value of ctio_cosmic_sig
...
; 26-Oct-2011 AT Remove common, add gain keyword
...
;if(keyword_set(sig)) then xsd = sig else xsd=ctio_cosmic_sig 
if keyword_set(sig) then xsd = sig else xsd=5
...
if ~keyword_set(gain) then gain = 1                 ;get the gain
if keyword_set(ron) then pbg=ron else pbg = 0                 ;get the gain
;pbg = ctio_pbg                   ;get the read noise/other sources of constant noise
...
;if keyword_set(stop) then stop      
  ...
  negative =  where (tmp lt 0,  nneg) 

  if nneg gt 0 then begin
    if n_elements(tmp) gt 1 then $ 
      tmp[negative] = mean(int[0, ido[negative]+[-1, 1]]) $
    else tmp =  mean(int[0, ido+[-1, 1]])
  endif
...
rep = where(potcos gt 0. and potcos lt 5., nrep)
