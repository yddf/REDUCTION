...
;17-may-08 DAF adapt for CTIO
; 13-Oct-2011 AT Remove common
...
;     print, 'i= '+i
 ...
 ;      plot, trough(*,i-1),psym=3, yrange=[-150,150]
       xs = ix
;       if i eq 21 or i eq 22 then remove,indgen(150)+925,xs ; commented HTI looks telescope specific.
       remove,where(xs mod 4 ne 0),xs
       splinefit = bspline_iterfit(xs, trough(xs,i-1), bkspace = 200, upper = 2, lower = 2, /silent) 
...
;Now loop through orders, subtracting sky.
  ...
  print,'GETSKY: Sky subtracted - returning to caller.'
