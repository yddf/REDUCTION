...
symbol_list = [ $
  ...
  'CONVOL':  BEGIN
               n= 41.0
               theta= findgen(n)/(n-1.0) * 405.0 * !dtor
               edg = 1d/sqrt(2)
               x= [sin(theta),0, -edg, 0, -edg,    0, edg]
               y= [cos(theta),0,  edg, 0, -edg,   0, -edg]
             END
  'DIAMOND': BEGIN
  ...
  'OBSERVER':  BEGIN
  			   nel=40d
               theta= (dindgen(nel+1d)/nel*30.0)*!dtor
               x= [0d, 5d * sin(30d * !dtor), 0d, 0d, 0d, $
               2.5d * sin(theta), 0.0]
               y= [0d, 5d * cos(30d * !dtor), 0d, 5d, 2.5d, $
               2.5d * cos(theta) ,0.0]
             END
  'PACMAN':  BEGIN
...
;
  USERSYM,x,y,_EXTRA=_extra, thick=thick, fill=fill
