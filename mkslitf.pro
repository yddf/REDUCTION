pro mkslitf, im, back, ymin, ycen, ymax, yslitf, slitf, sfunc, bincen, plot=plot, mask = mask, fwhm = fwhm, gain=gain
...
; 17-May-09 DAF   Adapted for CTIO
; Oct 26 2011 AT remove common block. Add gain keyword. Assume im and sky are in photons
; if gain is not given 
;@ctio.common
...
if ~keyword_set(gain) then gain = 1.  ; default gain
...
;Fit a smooth curve through the data with a spline.
  ...
  for bkspace = 0.5, 1, .1 do begin
    hist = histogram(ysf, binsize = bkspace)
    Nh = n_elements(hist)
    hist = hist[0:Nh-2]
    if min(hist) gt max(hist)*0.6 then break
  endfor

  splinefit = bspline_iterfit(ysf, msf, bkspace = bkspace, outmask = good, invvar = invvar,  upper = 3, lower = 3, /silent) 
...
;Spline onto final slit function grid.
  ...
  inf = where(finite(sfunc(*, i)) eq 0, ni)
  if ni gt 0 then sfunc(inf, i) = 0.
...
fwhm = fwhm + fwhm1(slitf(*, i))  ;problem exists here when fwhm has too may meanings.
